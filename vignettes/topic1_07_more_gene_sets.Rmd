...
Besides using `query()` to search for the AnnotationHub dataset, there are another two
ways which allows interactively searching for datasets:

- Using the **BiocHubsShiny** package.
- Using `ah_shiny()` function from the **GSEATraining** package.
...
With an `OrgDb` object, you can also use the helper function `get_GO_gene_sets_from_orgdb()` in **GSEAtraning** package:
