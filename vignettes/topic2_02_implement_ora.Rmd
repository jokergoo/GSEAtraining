...
Implementing ORA is very simple. As long as genes and gene sets are already in the correct ID types and format, the implementation of ORA only needs several lines.
...
1. set the default of `universe` as the total genes in `gene_sets`
2. intersect `genes` and `gene_sets` to `universe`. Note this step also removes duplicated genes
3. filter the result by the minimal number of DE genes in gene sets
4. add adjusted log2 fold enrichment and p-values
5. order the result table by the significance
...
gene lists using the GO BP gene sets and compare the ORA results (the whatever comparison
you can think of).
...
Grt the GO BP gene sets. Note genes in `de_genes_*` are symbols, thus genes in gene sets
...
Actually we can see gene set enrichment analysis is more robust than DE analysis.
