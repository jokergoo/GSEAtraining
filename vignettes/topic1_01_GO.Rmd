...
Only we only see the GO IDs, the object `GOTERM` can be used to get the name and long description
of GO terms.
...
Merge all offspring terms:

```{r}
lt2 = lapply(lt2, function(x) {
    unique(unlist(lt[x]))
})
```
...
```{r}
lt3 = as.list(org.Hs.egGO2ALLEGS)
lt3 = lapply(lt3, unique)

cn = intersect(names(lt2), names(lt3))
plot(sapply(lt2[cn], length), sapply(lt3[cn], length), log = "xy",
