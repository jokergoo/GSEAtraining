...
We calculate the cummulative probability of a gene being in a gene set $G$. At position $p$ in the
...
Here in the calculatio of `f1`, `sum(l_set)` is $n_k$, `cumsum(l_set)` is $\sum_{i=1}^p I(g_i \in G_k)$.
...
So, a large value of `f1 - f21` means the genes are more like to be in the gene set for those locating at `1..p`.
...
# s should be sorted
calculate_es_v2_gene_perm = function(s, geneset, perm = FALSE, power = 1) {
...
```{r}
es = calculate_es_v2_gene_perm(s, geneset)
es_rand = numeric(1000)
for(i in 1:1000) {
...
You can also call NES as fold enrichment, the log2 fold enrichment is also widely used:

```{r}
log2(es/mean(es_rand))
```
...
plot(df1$z, df2$z, main = "Z-score", xlim = c(0, 4), ylim = c(0, 4))
